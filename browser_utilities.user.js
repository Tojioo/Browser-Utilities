...
// @namespace    https://github.com/Tojioo/Browser-Utilities
// @version      7.2.9
// @author       Tojioo
...
const THEMES = {
    ...
    let previewBarPos    = 'top';     // 'top' | 'bottom'
    let previewBarAlign  = 'right';   // 'left' | 'right'
    ...
    function applyConfig(cfg) {
        ...
        if (['all','missing-alt','has-alt','linked','unlinked','cross','same','large','small'].includes(cfg.imgFilter)) imgFilter = cfg.imgFilter;
        ...
        if (['top','bottom'].includes(cfg.previewBarPos))          previewBarPos    = cfg.previewBarPos;
        if (['left','right'].includes(cfg.previewBarAlign))        previewBarAlign  = cfg.previewBarAlign;
    }
    try { applyConfig(JSON.parse(localStorage.getItem('su-config') || '{}')); } catch (e) {}
    collapsed = readStore('su-collapsed');

    function persist() {
        const data = JSON.stringify({ pinned, corner, codeBlockWrap, cookieMode, theme, customHex, customColors, downloadSource, downloadLocation, previewBarPos, previewBarAlign, imgSortKey: imgSort.key, imgSortDir: imgSort.dir, imgFilter, imgView, sheetH });
    ...
    style.textContent = `
        #su-root, #su-root *, .su-popover, .su-popover *, #su-preview, #su-preview * {
            box-sizing: border-box !important; font-family: -apple-system, 'SF Pro Text', sans-serif !important;
            text-transform: none !important; letter-spacing: normal !important; word-spacing: normal !important;
        }
        #su-root button, #su-root input, #su-root select, #su-root textarea,
        .su-popover button, #su-preview button {
            -webkit-appearance: none !important; appearance: none !important;
            outline: none !important; text-decoration: none !important;
        }
        #su-root svg, .su-popover svg, #su-preview svg { overflow: visible; max-width: none; max-height: none; flex-shrink: 0; }
        #su-fab {
        ...
        .su-item {
            ...
            color: #f0f0f0; font-size: 13px !important; font-weight: 500 !important; cursor: pointer;
            border: none; background: none; width: 100%; text-align: left;
            transition: background .12s ease; line-height: 1.2 !important; -webkit-tap-highlight-color: transparent;
        }
        ...
        .su-item-sublabel { font-size: 11px !important; color: #888; font-weight: 400 !important; display: block; margin-top: 1px; }
        ...
        .su-vtitle { font-size: 15px !important; font-weight: 600 !important; color: #f0f0f0; letter-spacing: -0.01em !important; }
        ...
        .su-x {
            ...
            border: none; cursor: pointer; color: #aaa; font-size: 14px !important;
        ...
        .su-section-title { font-size: 13px !important; font-weight: 700 !important; letter-spacing: 0 !important; text-transform: none !important; color: #ededed; }
        ...
        .su-row-key { font-size: 12px !important; color: #666; font-weight: 500 !important; flex-shrink: 0; padding-top: 1px; }
        .su-row-val { font-size: 12px !important; color: #ccc; text-align: right; word-break: break-all; font-family: 'SF Mono','Menlo',monospace; }
        ...
        .su-subitem { font-size: 11px !important; color: #9ab; font-family: 'SF Mono','Menlo',monospace; padding: 4px 0 4px 12px; word-break: break-all; border-bottom: 1px solid rgba(255,255,255,0.04); }
        .su-subitem:last-child { border-bottom: none; }

        .su-chip { display: inline-block; padding: 2px 7px; border-radius: 5px; font-size: 11px !important; font-weight: 600 !important; font-family: -apple-system,sans-serif; }
        ...
        .su-browse {
            ...
            color: #ddd; font-size: 12px !important; font-weight: 600 !important; font-family: inherit; -webkit-tap-highlight-color: transparent;
        }
        .su-browse:hover { background: rgba(255,255,255,0.09); }

        /* Image browser */
        .su-img-cap { font-size: 11px !important; color: #777; padding: 10px 18px; }
        .su-img {
        ...
        .su-img-dims { font-size: 12px !important; color: #9ab; font-family: 'SF Mono','Menlo',monospace; }
        .su-img-host { font-size: 11px !important; color: #777; font-family: 'SF Mono','Menlo',monospace; word-break: break-all; margin-top: 2px; }
        .su-img-alt  { font-size: 11px !important; color: #888; margin-top: 2px; }
        .su-img-alt.muted { color: #555; font-style: italic !important; }
        .su-img-load { font-size: 11px !important; color: #666; margin-top: 2px; }
        .su-img-thumb {
        ...
        .su-cell-badge { position: absolute; top: 4px; left: 4px; background: rgba(0,0,0,0.6); color: #fff; font-size: 10px !important; font-weight: 700 !important; padding: 1px 6px; border-radius: 6px; font-family: -apple-system, sans-serif; }
        .su-cell-linked { position: absolute; top: 4px; right: 4px; background: var(--su-accent); color: #fff; font-size: 9px !important; font-weight: 700 !important; padding: 1px 5px; border-radius: 6px; font-family: -apple-system, sans-serif; }
        .su-linked-tag { display: inline-block; margin-top: 2px; padding: 1px 6px; border-radius: 5px; font-size: 10px !important; font-weight: 700 !important; background: var(--su-accent-18); color: var(--su-accent); }
        ...
        .su-group-title { font-size: 13px !important; font-weight: 700 !important; letter-spacing: 0 !important; text-transform: none !important; color: #ededed; padding: 0 18px; margin-bottom: 8px; }
        ...
        .su-set-label { font-size: 13px !important; color: #e8e8e8; line-height: 1.3 !important; }
        .su-set-sub { font-size: 11px !important; color: #888; margin-top: 2px; line-height: 1.3 !important; }
        ...
        .su-seg-btn { flex: 1; padding: 8px 6px; border: none; background: none; color: #aaa; font-size: 12px !important; font-weight: 600 !important; border-radius: 7px; cursor: pointer; font-family: inherit; white-space: nowrap; -webkit-tap-highlight-color: transparent; }
        .su-seg-btn.active { background: var(--su-accent); color: #fff; }

        .su-link-btn { background: none; border: 1px solid rgba(255,255,255,0.12); color: #bbb; border-radius: 9px; padding: 10px 12px; font-size: 12px !important; font-weight: 600 !important; cursor: pointer; width: 100%; text-align: left; font-family: inherit; -webkit-tap-highlight-color: transparent; }
        ...
        .su-hex-input { flex: 1; min-width: 0; background: rgba(255,255,255,0.06); border: 1px solid rgba(255,255,255,0.12); border-radius: 9px; color: #eee; padding: 9px 11px; font-size: 13px !important; font-family: 'SF Mono','Menlo',monospace; }
        .su-hex-input:focus { outline: none; border-color: var(--su-accent); }
        .su-swatch-add { display: flex; align-items: center; justify-content: center; color: #bbb; font-size: 22px !important; font-weight: 300 !important; line-height: 1 !important; background: rgba(255,255,255,0.05); border: 1.5px dashed rgba(255,255,255,0.3); }
        ...
        .su-btn { padding: 9px 16px; border-radius: 9px; border: none; font-size: 13px !important; font-weight: 600 !important; cursor: pointer; font-family: inherit; -webkit-tap-highlight-color: transparent; }
        ...
        .su-popover-item { display: flex; align-items: center; gap: 9px; width: 100%; padding: 10px 14px; background: none; border: none; color: #ddd; font-size: 12px !important; text-align: left; cursor: pointer; font-family: inherit; -webkit-tap-highlight-color: transparent; }
        ...
        .su-pop-tail { margin-left: auto; color: #888; font-size: 11px !important; }
        .su-popover-info { padding: 12px 14px; font-size: 12px !important; color: #cfcfcf; line-height: 1.5 !important; }
        ...
        #su-preview-bar { order: 0; flex: 0 0 auto; display: flex; align-items: center; justify-content: flex-end; gap: 8px; padding: 12px 14px; background: rgba(20,20,20,0.6); }
        #su-preview[data-bar-pos="bottom"] #su-preview-bar { order: 4; }
        #su-preview[data-bar-align="left"] #su-preview-bar { flex-direction: row-reverse; justify-content: flex-start; }
        #su-preview-meta { order: 1; flex: 0 0 auto; text-align: center; padding: 12px 16px 2px; font-size: 14px !important; color: #9a9a9a; font-family: 'SF Mono','Menlo',monospace; }
        #su-preview-meta b { color: #fff; font-weight: 700 !important; }
        #su-preview-strip { order: 2; flex: 0 0 auto; display: none; gap: 10px; overflow-x: auto; -webkit-overflow-scrolling: touch; overscroll-behavior: contain; padding: 10px 14px; background: rgba(20,20,20,0.4); }
        ...
        .su-prev-thumb span { font-size: 10px !important; color: #888; font-family: 'SF Mono','Menlo',monospace; }
        .su-prev-thumb.active span { color: var(--su-accent); }
        #su-preview-stage { order: 3; flex: 1 1 auto; overflow: auto; -webkit-overflow-scrolling: touch; overscroll-behavior: contain; display: flex; align-items: center; justify-content: center; padding: 16px; }
        #su-preview-img { display: block; max-width: 100%; max-height: 100%; object-fit: contain; background: rgba(255,255,255,0.03); }
        .su-prev-btn { display: flex; align-items: center; gap: 7px; background: rgba(255,255,255,0.12); border: none; color: #fff; border-radius: 8px; padding: 8px 12px; font-size: 12px !important; font-weight: 600 !important; cursor: pointer; font-family: inherit; -webkit-tap-highlight-color: transparent; }
        .su-prev-btn:hover { background: rgba(255,255,255,0.2); }
    `;
    (document.head || document.documentElement).appendChild(style);

    // ─── Icons ─────────────────────────────────────────────────────────────
    const iconCopy    = `<svg width="14" height="14" style="width:14px;height:14px;flex-shrink:0;display:block" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg>`;
    const iconCopyC   = `<svg width="14" height="14" style="width:14px;height:14px;flex-shrink:0;display:block" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg>`;
    const iconDesc    = `<svg width="14" height="14" style="width:14px;height:14px;flex-shrink:0;display:block" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/><polyline points="10 9 9 9 8 9"/></svg>`;
    const iconInspect = `<svg width="14" height="14" style="width:14px;height:14px;flex-shrink:0;display:block" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/><line x1="11" y1="8" x2="11" y2="14"/><line x1="8" y1="11" x2="14" y2="11"/></svg>`;
    const iconGear    = `<svg width="14" height="14" style="width:14px;height:14px;flex-shrink:0;display:block" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="3"/><path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 1 1-2.83 2.83l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 0 1-4 0v-.09A1.65 1.65 0 0 0 9 19.4a1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 1 1-2.83-2.83l.06-.06a1.65 1.65 0 0 0 .33-1.82 1.65 1.65 0 0 0-1.51-1H3a2 2 0 0 1 0-4h.09A1.65 1.65 0 0 0 4.6 9a1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 1 1 2.83-2.83l.06.06a1.65 1.65 0 0 0 1.82.33H9a1.65 1.65 0 0 0 1-1.51V3a2 2 0 0 1 4 0v.09a1.65 1.65 0 0 0 1 1.51 1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 1 1 2.83 2.83l-.06.06a1.65 1.65 0 0 0-.33 1.82V9a1.65 1.65 0 0 0 1.51 1H21a2 2 0 0 1 0 4h-.09a1.65 1.65 0 0 0-1.51 1z"/></svg>`;
    const iconJson    = `<svg width="14" height="14" style="width:14px;height:14px;flex-shrink:0;display:block" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M8 3H7a2 2 0 0 0-2 2v4a2 2 0 0 1-2 2 2 2 0 0 1 2 2v4a2 2 0 0 0 2 2h1"/><path d="M16 3h1a2 2 0 0 1 2 2v4a2 2 0 0 0 2 2 2 2 0 0 0-2 2v4a2 2 0 0 1-2 2h-1"/></svg>`;
    const iconKebab   = `<svg width="14" height="14" style="width:14px;height:14px;flex-shrink:0;display:block" viewBox="0 0 24 24" fill="currentColor"><circle cx="12" cy="5" r="1.6"/><circle cx="12" cy="12" r="1.6"/><circle cx="12" cy="19" r="1.6"/></svg>`;
    const iconOverall = `<svg width="14" height="14" style="width:14px;height:14px;flex-shrink:0;display:block" viewBox="0 0 24 24" fill="currentColor"><circle cx="5" cy="12" r="1.7"/><circle cx="12" cy="12" r="1.7"/><circle cx="19" cy="12" r="1.7"/></svg>`;
    const iconInfo    = `<svg width="14" height="14" style="width:14px;height:14px;flex-shrink:0;display:block" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="16" x2="12" y2="12"/><line x1="12" y1="8" x2="12.01" y2="8"/></svg>`;
    const iconBack    = `<svg width="16" height="16" style="width:16px;height:16px;flex-shrink:0;display:block" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><polyline points="15 18 9 12 15 6"/></svg>`;
    const iconChevD   = `<svg class="su-chevron" width="12" height="12" style="width:12px;height:12px;flex-shrink:0;display:block" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.6" stroke-linecap="round" stroke-linejoin="round"><polyline points="6 9 12 15 18 9"/></svg>`;
    const iconChevR   = `<svg class="su-mini-chevron" width="12" height="12" style="width:12px;height:12px;flex-shrink:0;display:block" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.6" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 6 15 12 9 18"/></svg>`;
    const iconDownload= `<svg width="15" height="15" style="width:15px;height:15px;flex-shrink:0;display:block" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>`;
    const iconExternal= `<svg width="14" height="14" style="width:14px;height:14px;flex-shrink:0;display:block" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/></svg>`;
    const iconOrder   = `<svg width="15" height="15" style="width:15px;height:15px;flex-shrink:0;display:block" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M8 4v16"/><polyline points="4 8 8 4 12 8"/><path d="M16 20V4"/><polyline points="12 16 16 20 20 16"/></svg>`;
    const iconFilter  = `<svg width="15" height="15" style="width:15px;height:15px;flex-shrink:0;display:block" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="3" y1="6" x2="21" y2="6"/><line x1="6" y1="12" x2="18" y2="12"/><line x1="10" y1="18" x2="14" y2="18"/></svg>`;
    const iconImage   = `<svg width="14" height="14" style="width:14px;height:14px;flex-shrink:0;display:block" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/></svg>`;
    const iconLayout  = `<svg width="15" height="15" style="width:15px;height:15px;flex-shrink:0;display:block" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="7" height="7" rx="1"/><rect x="14" y="3" width="7" height="7" rx="1"/><rect x="3" y="14" width="7" height="7" rx="1"/><rect x="14" y="14" width="7" height="7" rx="1"/></svg>`;
    const iconClose   = `<svg width="14" height="14" style="width:14px;height:14px;flex-shrink:0;display:block" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round"><line x1="6" y1="6" x2="18" y2="18"/><line x1="18" y1="6" x2="6" y2="18"/></svg>`;
    const iconRefresh = `<svg width="15" height="15" style="width:15px;height:15px;flex-shrink:0;display:block" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="23 4 23 10 17 10"/><polyline points="1 20 1 14 7 14"/><path d="M3.51 9a9 9 0 0 1 14.85-3.36L23 10M1 14l4.64 4.36A9 9 0 0 0 20.49 15"/></svg>`;
    const iconGrid    = iconLayout;
    const iconList    = `<svg width="15" height="15" style="width:15px;height:15px;flex-shrink:0;display:block" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="8" y1="6" x2="21" y2="6"/><line x1="8" y1="12" x2="21" y2="12"/><line x1="8" y1="18" x2="21" y2="18"/><line x1="3" y1="6" x2="3.01" y2="6"/><line x1="3" y1="12" x2="3.01" y2="12"/><line x1="3" y1="18" x2="3.01" y2="18"/></svg>`;
    const iconCompact = `<svg width="15" height="15" style="width:15px;height:15px;flex-shrink:0;display:block" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="4" y1="5" x2="20" y2="5"/><line x1="4" y1="9" x2="20" y2="9"/><line x1="4" y1="13" x2="20" y2="13"/><line x1="4" y1="17" x2="20" y2="17"/></svg>`;
    const iconArrowUp = `<svg width="15" height="15" style="width:15px;height:15px;flex-shrink:0;display:block" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="12" y1="20" x2="12" y2="5"/><polyline points="6 11 12 5 18 11"/></svg>`;
    const iconArrowDown=`<svg width="15" height="15" style="width:15px;height:15px;flex-shrink:0;display:block" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="12" y1="4" x2="12" y2="19"/><polyline points="6 13 12 19 18 13"/></svg>`;
    ...
    fab.innerHTML = `<svg width="18" height="14" style="width:18px;height:14px;flex-shrink:0;display:block" viewBox="0 0 18 14"><rect x="0" y="0" width="18" height="2" rx="1" fill="white"/><rect x="0" y="6" width="18" height="2" rx="1" fill="white"/><rect x="0" y="12" width="18" height="2" rx="1" fill="white"/></svg><span id="su-fab-dot"></span>`;
    ...
    preview.innerHTML = `
        <div id="su-preview-bar">
            ...
            <button class="su-prev-btn" id="su-prev-linked" data-action="prev-linked" style="display:none">${iconExternal}<span>Open Linked</span></button>
    ...
    function applyPreviewBar() { preview.dataset.barPos = previewBarPos; preview.dataset.barAlign = previewBarAlign; }
    applyPreviewBar();
    ...
    function gatherSections() {
        ...
        for (let i = 0; i < all.length; i++) {
            ...
            if (tag === 'IMG') {
                ...
                let linkedOriginal = null;
                const wrapLink = el.closest('a[href]');
                if (wrapLink) {
                    try {
                        const lu = new URL(wrapLink.getAttribute('href'), loc.href);
                        if (lu.href !== src && /\.(jpe?g|png|gif|webp|avif|bmp|svg|tiff?)$/i.test(lu.pathname)) linkedOriginal = lu.href;
                    } catch (e) {}
                }
                if (imageEntries.length < IMG_VIEW_CAP && src) {
                    const r = el.getBoundingClientRect();
                    imageEntries.push({
                        el, src, origin, linkedOriginal,
    ...
    // A count in sectionMap is a display shortcut for a list computed on demand by detailItems;
    // copies must resolve that list instead of exporting the count that stands in for it.
    function resolvedValue(section, key, storedVal) {
        const fn = detailItems(section, key);
        return fn ? fn() : storedVal;
    }
    function resolvedSection(name) {
        const data = sectionMap[name], out = {};
        for (const key of Object.keys(data)) out[key] = resolvedValue(name, key, data[key]);
        if (name === 'Images') out['Image URLs'] = imageUrlsExport();
        return out;
    }
    // Per-image detail for JSON export only; the inspector's Images section shows counts,
    // full per-image data lives in the image browser instead.
    function imageUrlsExport() {
        return imageGroups().map(g => ({
            Id: g.id,
            Origin: g.src,
            LinkedOriginal: g.linkedOriginal || null,
            Alt: g.alt || null,
            NaturalSize: (g.nw && g.nh) ? `${g.nw}x${g.nh}` : null,
            RenderedSizes: g.variants.map(v => `${v.rw}x${v.rh}`),
        }));
    }
    function resolvedAll() {
        const out = {};
        for (const name of Object.keys(sectionMap)) out[name] = resolvedSection(name);
        return out;
    }
    ...
    function imageGroups() {
        ...
        for (const im of imageEntries) {
            ...
            if (!g) { g = { id: map.size, src: im.src, origin: im.origin, alt: im.alt, nw: im.nw, nh: im.nh, natural: im.natural, load: im.load, firstIdx: im.idx, linkedOriginal: null, variants: [] }; map.set(im.src, g); }
            if (!g.alt && im.alt) g.alt = im.alt;
            if (!g.linkedOriginal && im.linkedOriginal) g.linkedOriginal = im.linkedOriginal;
    ...
    function sortedFilteredGroups() {
        ...
        const list = imageGroups().filter(g => {
            switch (imgFilter) {
                ...
                case 'linked':      return !!g.linkedOriginal;
                case 'unlinked':    return !g.linkedOriginal;
        ...
        return `
            <button class="su-img" data-action="img-entry" data-id="${g.id}">
                <span class="su-img-info">
                    ...
                    ${g.linkedOriginal ? `<span class="su-linked-tag" title="${escapeHtml(g.linkedOriginal)}">Linked</span>` : ''}
    ...
    function imgRowCompact(g) {
        return `
            <button class="su-img compact" data-action="img-entry" data-id="${g.id}">
                <img class="su-img-thumb" src="${escapeHtml(g.src)}" loading="lazy" referrerpolicy="no-referrer" alt="">
                <span class="su-img-info">
                    <span class="su-img-dims">${escapeHtml(g.natural)}${g.variants.length > 1 ? ` · ${g.variants.length}×` : ''}${g.linkedOriginal ? ' · Linked' : ''}</span>
                    <span class="su-img-host">${escapeHtml(g.origin || '(relative)')}</span>
                </span>
                <span class="su-icon-btn su-img-dl" data-action="img-download" data-id="${g.id}" role="button" title="Download">${iconDownload}</span>
            </button>`;
    ...
    function imgCellGrid(g) {
        return `
            <button class="su-img-cell${g.alt ? '' : ' noalt'}" data-action="img-entry" data-id="${g.id}">
                ...
                ${g.linkedOriginal ? `<span class="su-cell-linked" title="${escapeHtml(g.linkedOriginal)}">Linked</span>` : ''}
    ...
    function filterLabel(v) { return ({ all: 'All images', 'missing-alt': 'Missing alt', 'has-alt': 'Has alt', linked: 'Linked', unlinked: 'Not linked', cross: 'Cross-origin', same: 'Same-origin', large: 'Large (256px+)', small: 'Small (under 64px)' })[v] || 'All images'; }
    function settingsBodyHtml() {
        ...
        return `
            ...
            <div class="su-group">
                ...
                <div class="su-set-col"><div class="su-set-label">Preview bar position</div>${seg('set-prevpos', previewBarPos, [{val:'top',label:'Top'},{val:'bottom',label:'Bottom'}])}<div class="su-set-sub" style="margin-top:8px">Where the Open origin, Open Linked, Download, and Close controls sit in the full image preview.</div></div>
                <div class="su-set-col"><div class="su-set-label">Preview bar alignment</div>${seg('set-prevalign', previewBarAlign, [{val:'right',label:'Right'},{val:'left',label:'Left'}])}<div class="su-set-sub" style="margin-top:8px">Left mirrors the button order and anchors the bar to the left edge.</div></div>
            </div>
            <div class="su-group">
    ...
    function resetDefaults() {
        ...
        previewBarPos = 'top'; previewBarAlign = 'right';
        imgSort = { key: 'dom', dir: 'asc' }; imgFilter = 'all'; imgView = 'list'; sheetH = 82; applySheetHeight();
        fab.classList.remove('unpinned'); applyCorner('br'); applyTheme('teal'); applyCookieMode(); applyPreviewBar(); persist();
    }
    ...
    popover.addEventListener('click', (e) => {
        ...
        if (a === 'copy-value') copyToClipboard(formatValue(resolvedValue(item.dataset.section, item.dataset.key, sectionMap[item.dataset.section][item.dataset.key])), item);
        else if (a === 'copy-all-json') copyToClipboard(wrapCopy(JSON.stringify(resolvedAll(), null, 2), 'json'), item);
    ...
    function openFilterMenu(anchor) {
        const opts = [['all','All'],['missing-alt','Missing alt'],['has-alt','Has alt'],['linked','Linked'],['unlinked','Not linked'],['cross','Cross-origin'],['same','Same-origin'],['large','Large (256px+)'],['small','Small (under 64px)']];
    ...
    function renderPreview() {
        ...
        preview.querySelector('#su-preview-meta').innerHTML = `<b>${v.rw} × ${v.rh}</b> shown${g.nw ? ` · source ${g.nw} × ${g.nh}` : ''}${g.linkedOriginal ? ` · <span class="su-linked-tag" title="${escapeHtml(g.linkedOriginal)}">Linked</span>` : ''}`;
        const linkedBtn = preview.querySelector('#su-prev-linked');
        if (linkedBtn) linkedBtn.style.display = g.linkedOriginal ? '' : 'none';
    ...
    preview.addEventListener('click', (e) => {
        ...
        else if (a === 'prev-linked') { if (previewGroup && previewGroup.linkedOriginal) window.open(previewGroup.linkedOriginal, '_blank'); }
    ...
    async function downloadImage(g, variant) {
        ...
        try {
            ...
            const url = g.linkedOriginal || g.src;
            const resp = await fetch(url, { mode: 'cors' });
            await deliver(await resp.blob(), fileName(url));
        } catch (e) {
            window.open(g.linkedOriginal || g.src, '_blank');  // fallback: let the user save manually
        }
    }
    ...
    viewsEl.addEventListener('click', (e) => {
        ...
        switch (act.dataset.action) {
            ...
            case 'section-json': copyToClipboard(wrapCopy(JSON.stringify(resolvedSection(act.dataset.section), null, 2), 'json'), act); break;
            ...
            case 'set-dlloc': downloadLocation = act.dataset.val; persist(); refreshSettings(); break;
                case 'set-prevpos': previewBarPos = act.dataset.val; persist(); applyPreviewBar(); refreshSettings(); break;
                case 'set-prevalign': previewBarAlign = act.dataset.val; persist(); applyPreviewBar(); refreshSettings(); break;
